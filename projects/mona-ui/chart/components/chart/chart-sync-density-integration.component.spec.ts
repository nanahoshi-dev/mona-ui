...
describe("combined synchronization + density stress matrix", () => {
    ...
    it("one source viewport operation produces one bounded recipient projection and no Stage A/B", async () => {
        ...
        try {
            ...
            host.chartA().setViewportWindow({ axis: "x", axisId: "x-main", kind: "continuous", max: 90_000, min: 10_000 });
            await nextFrame();
            ...
            const bViewport = host.chartB().getViewport()?.axes.find(a => a.axisId === "x-main") as
