...
import { RangeBarSeriesComponent } from "../components/range-bar-series/range-bar-series.component";
    ...
    template: `
        <div style="width: 600px; height: 400px;">
            <mona-chart
                ...
                @if (showArea()) {
                    <mona-area-series [field]="'revenue'" [name]="'Revenue Area'" [showPoints]="true" />
                }
...
describe("Horizontal Bar Chart Integration", () => {
    ...
    it("animates when toggling the last visible horizontal bar series to hidden", () => {
        ...
        const barSeries1 = fixture.debugElement.queryAll(By.directive(BarSeriesComponent))[0]
            .componentInstance as BarSeriesComponent;
        const barSeries2 = fixture.debugElement.queryAll(By.directive(BarSeriesComponent))[1]
            .componentInstance as BarSeriesComponent;
    ...
    it("correctly integrates zero-length horizontal range bar with vertical hairline renderer (HAX-3-001, HAX-3-002)", () => {
        ...
        expect(mockCtx.lineTo).toHaveBeenCalledWith(
            expect.any(Number),
            rangeBarScene.bars[0].y + rangeBarScene.bars[0].height
        );
    ...
    it("preserves all Y-axis category labels without collision thinning when categories fit within plot height", () => {
        ...
        expect(yAxisScene!.ticks.map(t => t.formattedValue)).toEqual(["Jan", "Feb", "Mar", "Apr", "May", "Jun"]);
    });
});
