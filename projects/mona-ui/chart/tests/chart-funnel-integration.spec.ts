...
import { FunnelSeriesComponent } from "../components/funnel-series/funnel-series.component";
        ...
        <mona-chart [style.width.px]="500" [style.height.px]="400">
            <mona-funnel-series
                ...
                <ng-template monaChartFunnelLabelTemplate let-stage>
                    <span class="custom-funnel-label"
                        >{{ stage.category }}: {{ stage.value }} ({{ stage.formattedOverallConversionRate }})</span
                    >
                </ng-template>
            </mona-funnel-series>
        </mona-chart>
...
describe("ChartComponent Funnel Integration", () => {
    ...
    it("renders default funnel DOM labels with configurable labelContent", () => {
        ...
        class DefaultFunnelTestComponent {
            ...
            public readonly labelContent = signal<
                "category" | "value" | "category-value" | "category-value-conversion"
            >("category-value");
        }
                    ...
                    <mona-funnel-series
                        ...
                        <ng-template monaChartFunnelLabelTemplate let-stage>
                            <span
                                ...
                                [attr.data-fill]="stage.color"
                                [attr.data-text]="stage.textColor">
                                {{ stage.category }}
                            </span>
                        </ng-template>
                    </mona-funnel-series>
    ...
    it("announces visible stage position, overall conversion, and positive-only drop-off in live region (FWF-C6, FWF-C7)", () => {
        @Component({
            ...
            template: `
                <mona-chart [animation]="false" [style.width.px]="500" [style.height.px]="400">
                    <mona-funnel-series [data]="data" [field]="'value'" [categoryField]="'stage'" [name]="'Pipeline'" />
                </mona-chart>
            `
        })
        ...
        fixture.detectChanges();
        expect(liveRegion.textContent?.trim()).toBe("Pipeline, Visits, stage 1 of 3: 2000. Overall conversion 100%.");
