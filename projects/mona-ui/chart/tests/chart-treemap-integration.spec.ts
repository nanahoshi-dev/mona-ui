...
@Component({
    imports: [ChartComponent, TreemapSeriesComponent, ChartLegendComponent, ChartTooltipComponent],
    template: `
        <mona-chart style="width: 600px; height: 400px;" [animation]="false" (pointClick)="onPointClick($event)">
            <mona-treemap-series
                [data]="data()"
                [field]="'value'"
                [labelField]="'name'"
...
@Component({
    imports: [ChartComponent, TreemapSeriesComponent, LineSeriesComponent],
    template: `
...
class TestMixedChartComponent {
    public readonly data = signal<readonly TreemapTestItem[]>([{ name: "Root", value: 100 }]);
}
...
describe("ChartTreemapIntegration", () => {
    beforeEach(() => {
        TestBed.configureTestingModule({
            imports: [TestTreemapChartComponent, TestMixedChartComponent, TestMultiTreemapChartComponent]
        });
    });
    ...
    it("supports keyboard navigation with Arrow keys, Home, End, and triggers pointClick on Enter/Space", () => {
        ...
        const chart = fixture.debugElement.children[0].componentInstance as unknown as {
