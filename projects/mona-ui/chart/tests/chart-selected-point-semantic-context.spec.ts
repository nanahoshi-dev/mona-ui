...
@Component({
    ...
    template: `
        <mona-chart [data]="data()" [xField]="'category'" [style.width.px]="600" [style.height.px]="400">
            <mona-chart-x-axis />
            <mona-chart-y-axis [axisId]="'yPrimary'" />
            <mona-bar-series [field]="'s1'" [name]="'Series 1'" [stack]="'normal'" [yAxisId]="'yPrimary'" />
            <mona-bar-series [field]="'s2'" [name]="'Series 2'" [stack]="'normal'" [yAxisId]="'yPrimary'" />
...
describe("Chart Selected Point Semantic Context", () => {
    ...
    describe("Real Stacked Component Bed Integration", () => {
        ...
        it("emits exact normal stacked scalar properties and maintains data-label parity for real generated scene", async () => {
            ...
            const hitTarget = scene.hitTargets.find(
                t => t.seriesName === "Series 2" && (t.xKey === "2026-Q1" || t.category === "2026-Q1")
            );
