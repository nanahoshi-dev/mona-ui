...
describe("FunnelKeyboardNavigation", () => {
    function createMockScene(orientation: "horizontal" | "vertical" = "vertical"): CartesianFunnelChartScene {
        ...
        return {
            ...
            height: 300,
            hitIndex: new FunnelHitIndex({ height: 300, width: 200, x: 0, y: 0 }, orientation, 90, 10, [], hitTargets),
            hitTargets,
            interactionBuckets: [],
            legendItems: [],
            orientation,
    ...
    it("navigates vertically with ArrowDown / ArrowUp / Home / End", () => {
        ...
        const r1 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "ArrowDown" }), scene, -1);
        expect(r1?.bucketIndex).toBe(0);

        // ArrowDown from 0 -> 1
        const r2 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "ArrowDown" }), scene, 0);
        expect(r2?.bucketIndex).toBe(1);

        // ArrowDown from 2 -> clamps at 2
        const r3 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "ArrowDown" }), scene, 2);
        expect(r3?.bucketIndex).toBe(2);

        // ArrowUp from 2 -> 1
        const r4 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "ArrowUp" }), scene, 2);
        expect(r4?.bucketIndex).toBe(1);

        // Home -> 0
        const r5 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "Home" }), scene, 2);
        expect(r5?.bucketIndex).toBe(0);

        // End -> 2
        const r6 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "End" }), scene, 0);
    ...
    it("navigates horizontally with ArrowRight / ArrowLeft / Home / End", () => {
        ...
        const r1 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "ArrowRight" }), scene, -1);
        expect(r1?.bucketIndex).toBe(0);

        // ArrowRight from 0 -> 1
        const r2 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "ArrowRight" }), scene, 0);
        expect(r2?.bucketIndex).toBe(1);

        // ArrowLeft from 1 -> 0
        const r3 = FunnelKeyboardNavigation.handleKeyDown(new KeyboardEvent("keydown", { key: "ArrowLeft" }), scene, 1);
