...
9. Accessibility & Forms Integration
    ...
    * Form Interaction:
        ...
        * **Signal forms terminology:** The binding that connects a component to a signal form field is `[formField]`, not `[control]`. The interface a component implements is `FormCheckboxControl` (for checkboxes) or `FormValueControl<T>` (for other value types), both from `@angular/forms/signals`. `ControlValueAccessor` is the separate classic Angular forms interface — they are not interchangeable.
        * **`ControlValueAccessor` vs signal forms:** A component that only implements `FormCheckboxControl` / `FormValueControl<T>` (no `NG_VALUE_ACCESSOR` provider, no `writeValue`/`registerOnChange`/`registerOnTouched` methods) cannot be used with `[(ngModel)]`, `[formControl]`, or `formControlName`. Always verify which interface(s) are implemented before documenting forms support.
