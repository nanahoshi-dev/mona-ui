...
- In the tester app, follow the existing `loadComponent` route pattern in `projects/mona-ui-tester/src/app/routes.ts` for docs/demo pages.
- Use workspace scripts for workflow changes: `build`/`watch` run `build:metadata` first; run `npm run test:lib` and `npm run test:app` for project tests.
...
- When adding/removing public APIs, update `projects/mona-ui/src/lib/index.ts` exports (the package surface is re-exported via `projects/mona-ui/src/public-api.ts`).
